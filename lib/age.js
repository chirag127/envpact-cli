...
function decryptValue(wrapped) {
  ...
  // Return age's output verbatim — do NOT strip trailing newlines.
  // The plaintext fed to encryptValue is the canonical value; we
  // preserve it byte-for-byte. (Earlier versions stripped one
  // trailing newline, which truncated values that legitimately
  // ended with multiple newlines, e.g. multi-line PEM blocks.)
  return r.stdout;
}
